contract BonusContract {
    ...
    */
    function calculateBonus(uint _toknes) public view returns (uint bonusTokens) {
...
contract BonusContract2 {
    ...
    */
    function calculateBonus(uint _toknes) public view returns (uint bonusTokens) {
        ...
        if(block.timestamp >= bonus[0].startTime && block.timestamp < bonus[0].endTime) {
            bonusTokens = _toknes.mul(bonus[0].bonusPercent).div(100);
        } else if(block.timestamp >= bonus[1].startTime && block.timestamp < bonus[1].endTime) {
            bonusTokens = _toknes.mul(bonus[1].bonusPercent).div(100);
        } else if(block.timestamp >= bonus[2].startTime && block.timestamp < bonus[2].endTime) {
            bonusTokens = _toknes.mul(bonus[2].bonusPercent).div(100);
        }
    }
}